...
class EnvAgent:
    ...
    release_date = attrib(default=0, type=int)
    deadline = attrib(default=2**30, type=int)
    ...
    def get_timing_penalty(self, elapsed_steps, max_episode_steps):
        ...
        if self.status == RailAgentStatus.ACTIVE:

            # Compute the number of steps the agent is outside bounds.
            steps_outside = 0

            if elapsed_steps <= self.release_date:
                steps_outside = 1 + self.release_date - elapsed_steps
            if elapsed_steps >= self.deadline:
                steps_outside = 1 + elapsed_steps - self.deadline
    ...
    def from_schedule(cls, schedule: Schedule):
        ...
        release_dates = [0] * len(schedule.agent_positions)
        deadlines = [2**30] * len(schedule.agent_positions)

        if schedule.release_dates:
            release_dates = schedule.release_dates
