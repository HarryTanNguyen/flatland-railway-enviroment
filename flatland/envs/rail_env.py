    ...
    def _step_agent(self, i_agent, action: Optional[RailEnvActions] = None):
        ...
        if agent.status == RailAgentStatus.READY_TO_DEPART:
            if action in [RailEnvActions.MOVE_LEFT, RailEnvActions.MOVE_RIGHT,
                ...
                self.rewards_dict[i_agent] += agent.get_timing_penalty(self._elapsed_steps, self._max_episode_steps)
                return
            else:
                # Only award schedule step penalty once the agent's release date has passed.
                if (agent.release_date < self._elapsed_steps):
                    self.rewards_dict[i_agent] += self.step_penalty * agent.speed_data['speed']
                return
        ...
        # Add activity penalty if agent operating outside its [release, deadline] window.
        self.rewards_dict[i_agent] += agent.get_timing_penalty(self._elapsed_steps, self._max_episode_steps)
