...
def complex_schedule_generator(speed_ratio_map: Mapping[float, float] = None, seed: int = 1, timed: bool = False) -> ScheduleGenerator:
    """
    ...
    def generator(rail: GridTransitionMap, num_agents: int, hints: Any = None, num_resets: int = 0,
        ...
        if timed:
            ...
            for sg in start_goal:
                ...
                manhattan = np.abs(start[0] - target[0]) + np.abs(start[1] - target[1])

                # Randomize release date and deadline 
                release_date = np_random.randint(0, 4 + 2*num_agents)
                deadline = release_date + manhattan + np_random.randint(int(manhattan/4), manhattan + 2*num_agents)
        ...
        return Schedule(agent_positions=agents_position, agent_directions=agents_direction,
                        ...
                        max_episode_steps=max_episode_steps, release_dates=release_dates, deadlines=deadlines)

    return generator


def sparse_schedule_generator(speed_ratio_map: Mapping[float, float] = None, seed: int = 1, timed: bool = False) -> ScheduleGenerator:
    """
    ...
    def generator(rail: GridTransitionMap, num_agents: int, hints: Any = None, num_resets: int = 0,
        ...
        for agent_idx in range(num_agents):
            ...
            # Adjust release dates and deadlines
            if timed:
                # Compute Manhattan distance from start to goal.
                manhattan = np.abs(start[0][0] - target[0][0]) + np.abs(start[0][1] - target[0][1])
...
def random_schedule_generator(speed_ratio_map: Optional[Mapping[float, float]] = None,
    ...
    def generator(rail: GridTransitionMap, num_agents: int, hints: Any = None, num_resets: int = 0,
        ...
        return Schedule(agent_positions=agents_position, agent_directions=agents_direction,
                        ...
                        max_episode_steps=max_episode_steps, release_dates=None, deadlines=None)
